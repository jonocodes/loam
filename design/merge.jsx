    ...
    >
      ...
      <div style={{ flex: 1, overflow: 'auto', display: 'flex', flexDirection: 'column', minWidth: 0 }}>
        ...
        <div style={{ flex: 1, overflow: 'auto' }}>
          {view === 'index' && (
            <LoamIndex theme={theme} onOpen={open} activeTag={activeTag} setActiveTag={setActiveTag} />
          )}
          {view === 'note' && (
            <LoamNote
              theme={theme}
      ...
      <SidebarSection theme={theme} title="recent">
        {recents.map((n) => (
          <SidebarRow
            ...
            dot
            label={n.title}
            accent={n.kind === 'essay' ? theme.accent : theme.accent2}
          />
        ))}
      </SidebarSection>
      ...
      <SidebarSection theme={theme} title="tags">
        <div style={{ padding: '2px 6px 0', display: 'flex', flexWrap: 'wrap', gap: 4 }}>
          {window.TAGS.map((t) => (
            ...
            >
              {t.name}
              <span style={{ opacity: 0.5, marginLeft: 5 }}>{t.count}</span>
            </button>
          ))}
        </div>
      </SidebarSection>
  ...
  return (
    ...
    >
      ...
      {view !== 'index' && (
        <>
          <span style={{ color: theme.faint }}>/</span>
          <span style={{ color: theme.ink, fontFamily: '"JetBrains Mono", ui-monospace, monospace', fontSize: 11 }}>
            {note.id}.md
          </span>
        </>
      )}
      ...
      {!activeTag ? (
        <div style={{ marginBottom: 36 }}>
          ...
          <p style={{ fontSize: 15, color: theme.dim, lineHeight: 1.6, margin: 0, maxWidth: 540 }}>
            A small notebook kept in the open. Some of these are essays I'm still figuring out. Some are notes that may
            stay notes forever.
          </p>
        </div>
      ) : (
        ...
        {filtered.map((n) => (
          ...
          >
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 14, marginBottom: 5 }}>
              ...
              <span style={{ fontSize: 18, fontWeight: 600, letterSpacing: -0.2, flex: 1 }}>{n.title}</span>
              <span
                style={{
                  color: theme.dim,
                  fontSize: 12,
            ...
            <div style={{ fontSize: 14, color: theme.dim, lineHeight: 1.55, paddingLeft: 70 }}>{n.excerpt}</div>
            <div style={{ paddingLeft: 70, marginTop: 8, display: 'flex', gap: 6, alignItems: 'center' }}>
              ...
              <span style={{ flex: 1 }} />
              <span style={{ fontSize: 11, color: theme.dim, fontFamily: '"JetBrains Mono", ui-monospace, monospace' }}>
    ...
    >
      <article>
        ...
        <div style={{ fontSize: 16, lineHeight: 1.7 }}>
          {note.body.map((b, i) => (
            <LoamBlock key={i} block={b} theme={theme} onOpen={onOpen} />
          ))}
        </div>

        <div style={{ marginTop: 44, display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center' }}>
          <span
            style={{ fontSize: 10, color: theme.dim, marginRight: 8, letterSpacing: 1.4, textTransform: 'uppercase' }}
          >
            filed under
          </span>
          {note.tags.map((t) => (
      ...
      <aside style={{ position: 'sticky', top: 0, alignSelf: 'start', paddingTop: 36 }}>
        ...
        <div>
          ...
          {window.NOTES.filter((n) => n.id !== note.id && n.tags.some((t) => note.tags.includes(t)))
            .slice(0, 3)
            .map((n) => (
              <button
                key={n.id}
              ...
              >
                ...
                <div
                  style={{ fontSize: 11, color: theme.dim, fontFamily: '"JetBrains Mono", ui-monospace, monospace' }}
                >
                  {n.tags.map((t) => `#${t}`).join(' ')}
                </div>
              </button>
...
function LoamBlock({ block, theme, onOpen }) {
      ...
      <ul style={{ margin: '0 0 18px', paddingLeft: 22, listStyle: 'none' }}>
        {block.items.map((it, i) => (
          <li key={i} style={{ margin: '0 0 7px', position: 'relative' }}>
            <span style={{ position: 'absolute', left: -16, color: theme.accent }}>·</span>
            {it}
          </li>
        ))}
      </ul>
  ...
  if (block.type === 'code')
    return (
      ...
      >
        <div
          style={{ fontSize: 10, color: theme.dim, marginBottom: 8, letterSpacing: 1.2, textTransform: 'uppercase' }}
        >
          {block.lang || 'code'}
        </div>
...
function loamSyntax(text, theme) {
  ...
  text.split('\n').forEach((line, idx) => {
    ...
    const parts = tokens.map((tok, j) => {
      ...
      if (m) {
        const color =
          m[1] === 'S' ? theme.accent2 : m[1] === 'K' ? theme.accent : m[1] === 'N' ? theme.accent : theme.dim
        return (
          <span key={`${idx}-${j}`} style={{ color, fontStyle: m[1] === 'C' ? 'italic' : 'normal' }}>
            {m[2]}
          </span>
        )
      }
...
function LoamMobile({
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, overflow: 'auto' }}>
        {view === 'index' && (
          <LoamIndexMobile theme={theme} onOpen={onOpen} activeTag={activeTag} setActiveTag={setActiveTag} />
        )}
        {view === 'note' && (
          <LoamNoteMobile
            theme={theme}
          ...
          >
            ...
            <div style={{ padding: '4px 8px' }}>
              <button
                ...
                style={sidebarButton(theme, false)}
              >
                <span style={{ color: theme.accent, width: 14, fontSize: 14, lineHeight: 1 }}>＋</span>
                <span>Plant a note</span>
              </button>
    ...
    <div style={{ padding: '20px 18px 30px' }}>
      ...
      <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap', marginBottom: 16, overflow: 'auto' }}>
        <button
          ...
          style={{
            background: !activeTag ? theme.ink : theme.tag,
            color: !activeTag ? theme.bg : theme.ink,
            border: 'none',
            padding: '4px 11px',
            borderRadius: 999,
            cursor: 'pointer',
        ...
        {filtered.map((n) => (
          ...
          >
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginBottom: 4 }}>
              ...
              <span style={{ flex: 1 }} />
              <span style={{ color: theme.dim, fontSize: 11, fontFamily: '"JetBrains Mono", ui-monospace, monospace' }}>
                {n.editedDate.replace(', 2025', '')}
              </span>
            </div>
