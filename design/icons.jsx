  ...
  return (
    <div className="stage">
      <div className="stage-header">
        <div className="stage-title">loam <em>· final mark</em></div>
        <div className="stage-meta">01 · slab L · paper</div>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 32, alignItems: "center" }}>
        <div style={{ padding: 24, border: "1px dashed var(--line)", borderRadius: 12, background: "oklch(0.985 0.006 80)", display: "flex", justifyContent: "center" }}>
          <FinalIcon size={360} />
        </div>
        <div style={{ padding: 24, border: "1px dashed oklch(0.32 0.02 60)", borderRadius: 12, background: "oklch(0.18 0.02 60)", display: "flex", justifyContent: "center" }}>
...
function Swatch({ color, label, hex }) {
  return (
    <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
      <div style={{ width: 32, height: 32, background: color, borderRadius: 4, border: "1px solid oklch(0.85 0.01 70 / 0.5)" }} />
      <div>
        <div style={{ fontFamily: "JetBrains Mono, monospace", fontSize: 10, letterSpacing: "0.08em", textTransform: "uppercase", color: "var(--muted)" }}>{label}</div>
...
function FinalLockups() {
  return (
    <div className="stage">
      <div className="stage-header">
        <div className="stage-title">Wordmark <em>· lockups</em></div>
        <div className="stage-meta">Newsreader italic · loam in lowercase</div>
      </div>

      {/* light card */}
      <div style={{ padding: 36, background: FINAL.bg, borderRadius: 12, border: "1px solid var(--line)", display: "flex", flexDirection: "column", gap: 28, alignItems: "flex-start" }}>
      ...
      {/* dark card */}
      <div style={{ padding: 36, background: FINAL.inkBg, borderRadius: 12, border: "1px solid oklch(0.32 0.02 60)", display: "flex", flexDirection: "column", gap: 28, alignItems: "flex-start" }}>
        <Wordmark dark iconSize={120} fontSize={88} />
        <Wordmark dark iconSize={72} fontSize={56} />
        <Wordmark dark iconSize={36} fontSize={28} />
      </div>

      <div style={{ fontSize: 13, color: "var(--muted)", lineHeight: 1.6, fontFamily: "Newsreader, serif", fontStyle: "italic", maxWidth: "60ch" }}>
        The wordmark is set in Newsreader italic, all lowercase, with the icon's height matching the x-height-plus-cap region of the type.
        Optical alignment was tuned by eye, not metrics — the gap is roughly 18% of the icon size.
      </div>
    </div>
  );
}
...
function FinalExports() {
  ...
  const exports = [
    { name: "loam-icon-1024.svg", label: "App icon · 1024", svg: () => buildLightSVG(1024), preview: <FinalIcon size={120} /> },
    { name: "loam-icon-1024-dark.svg", label: "App icon · 1024 (dark)", svg: () => buildDarkSVG(1024), preview: <FinalIcon size={120} dark /> },
    ...
    { name: "loam-favicon-32.svg", label: "Favicon · 32", svg: () => buildFaviconSVG(32), preview: <FinalFavicon size={48} /> },
    { name: "loam-favicon-16.svg", label: "Favicon · 16", svg: () => buildFaviconSVG(16), preview: <FinalFavicon size={32} /> },
  ];

  return (
    <div className="stage">
      <div className="stage-header">
        <div className="stage-title">Exports <em>· click to download SVG</em></div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16 }}>
        {exports.map(e => (
          <button
            key={e.name}
            onClick={() => { downloadSVG(e.svg(), e.name); setCopied(e.name); setTimeout(() => setCopied(null), 1200); }}
            style={{
          ...
          >
            ...
            <div style={{ fontFamily: "Newsreader, serif", fontSize: 14, fontStyle: "italic", color: "var(--ink)" }}>{e.label}</div>
            <div style={{ letterSpacing: "0.08em", textTransform: "uppercase", fontSize: 9, color: copied === e.name ? FINAL.accent : "var(--muted)" }}>
              {copied === e.name ? "downloaded ↓" : e.name}
            </div>
          </button>
        ))}
      </div>
      <div style={{ fontSize: 12, color: "var(--muted)", fontFamily: "JetBrains Mono, monospace", lineHeight: 1.6, marginTop: 6 }}>
...
function FinalSpec() {
  return (
    <div className="stage">
      <div className="stage-header">
        <div className="stage-title">Spec <em>· construction</em></div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "auto 1fr", gap: 32, alignItems: "start" }}>
        ...
        <div style={{ position: "relative", width: 360, height: 360 }}>
          <FinalIcon size={360} />
          {/* overlay: dimensions */}
          <svg viewBox="0 0 1024 1024" width="360" height="360" style={{ position: "absolute", top: 0, left: 0, pointerEvents: "none" }}>
            {/* L bounding box */}
            <rect x="320" y="200" width="440" height="630" fill="none" stroke={FINAL.accent} strokeWidth="3" strokeDasharray="8 6" opacity="0.7" />
            {/* dimension ticks */}
            <line x1="320" y1="180" x2="760" y2="180" stroke={FINAL.accent} strokeWidth="2" opacity="0.7" />
            <text x="540" y="170" textAnchor="middle" fontSize="22" fill={FINAL.accent} fontFamily="JetBrains Mono, monospace">440</text>
            <line x1="780" y1="200" x2="780" y2="830" stroke={FINAL.accent} strokeWidth="2" opacity="0.7" />
            <text x="800" y="520" fontSize="22" fill={FINAL.accent} fontFamily="JetBrains Mono, monospace">630</text>
            {/* seed */}
            <circle cx="690" cy="760" r="48" fill="none" stroke={FINAL.accent} strokeWidth="3" strokeDasharray="6 4" opacity="0.9" />
            <text x="690" y="900" textAnchor="middle" fontSize="20" fill={FINAL.accent} fontFamily="JetBrains Mono, monospace">r=48</text>
          </svg>
        </div>
        <div style={{ fontFamily: "Newsreader, serif", fontSize: 14, lineHeight: 1.7, color: "var(--ink)", maxWidth: "48ch" }}>
          <div style={{ fontFamily: "JetBrains Mono, monospace", fontSize: 10, letterSpacing: "0.14em", textTransform: "uppercase", color: "var(--muted)", marginBottom: 6 }}>geometry</div>
          <p style={{ margin: "0 0 14px" }}>
            The L is plotted on a 1024 grid. The vertical bar runs from <span className="mono">y=200</span> to <span className="mono">y=830</span>;
            the foot extends right to <span className="mono">x=760</span>. Stem width is <span className="mono">150</span>; foot height is <span className="mono">130</span>.
          </p>
          <p style={{ margin: "0 0 14px" }}>
            The seed is a circle with <span className="mono">r=48</span> placed at <span className="mono">(690, 760)</span> — sitting in the negative
            space at the corner of the foot, slightly off the L's terminal. It anchors the composition without
            crowding the letter.
          </p>
          <div style={{ fontFamily: "JetBrains Mono, monospace", fontSize: 10, letterSpacing: "0.14em", textTransform: "uppercase", color: "var(--muted)", margin: "18px 0 6px" }}>scaling rules</div>
          <p style={{ margin: 0 }}>
            At <span className="mono">≤ 32px</span> the icon switches to the favicon variant: a thicker L on a circular ground,
            with the seed enlarged to <span className="mono">r=68</span> and shifted to <span className="mono">(700, 780)</span> for visual balance at small sizes.
          </p>
        </div>
      </div>
    </div>
...
Object.assign(window, {
  FinalHero, FinalLockups, FinalExports, FinalSpec, FinalIcon, FinalFavicon,
});
